...
class Provider(Base):
    ...
    prov_id = Column(
        UUID, primary_key=True, index=True, comment='ID primário do fornecedor no banco'
    )
    ...
    prov_username = Column(
        String, comment='Nome do ERP que usa o SV Recommender. Exemplo: sv'
    )
    prov_password = Column(String, nullable=False, comment='Senha do fornecedor')
    prov_email = Column(String, nullable=False, comment='Senha do fornecedor')
    prov_hashed_password = Column(
        String, nullable=False, comment='Senha do fornecedor criptografada'
    )
    prov_token_str = Column(
        String, nullable=False, comment='Token de acesso do fornecedor'
    )
...
class Companies(Base):
    ...
    comp_id = Column(
        UUID, primary_key=True, index=True, comment='ID primário da empresa no banco'
    )
    comp_prov_id = Column(UUID, index=True, comment='ID do ERP que a empresa usa')
    comp_prco_id = Column(
        UUID, primary_key=True, index=True, comment='ID da empresa no ERP'
    )
...
class TransactionHistory(Base):
    ...
    trhi_id = Column(UUID, primary_key=True, nullable=False, index=True)
    ...
    trhi_tran_id = Column(Text, nullable=False, comment='Número do Pedido ou NFe')
    trhi_transated_at = Column(DateTime, nullable=False)
    trhi_prov_id = Column(
        Text, nullable=False, comment='ID do fornecedor que enviou o arquivo'
    )
    trhi_comp_id = Column(
        Text, nullable=False, comment='ID da empresa que enviou o arquivo'
    )
    trhi_salesperson_id = Column(Decimal, comment='ID do vendedor')
    trhi_clie_id = Column(Text, comment='ID do Consumidor (CNPJ, CPF, COD)')
    trhi_prod_sku = Column(Text, nullable=False, comment='SKU do produto')
    trhi_prod_sku_category = Column(Text, comment='Categoria do SKU do produto')
    trhi_prod_price = Column(Decimal, default=0, comment='Preço do produto')
    trhi_prod_quantity = Column(
        Decimal, default=1, comment='Quantidade de produtos vendidos'
    )
    trhi_prod_order = Column(Integer, default=0, comment='Ordem do produto no pedido')
    trhi_freight_type = Column(Text, default='', comment='Tipo de frete')
    trhi_freight_value = Column(Decimal, default=0, comment='Valor do frete')
    trhi_commission = Column(Decimal, default=0, comment='Comissão do vendedor')
    trhi_remarks = Column(
        Text, nullable=False, default='', comment='Observações sobre o pedido ou item'
    )
...
class TransactionHistoryFile(Base):
    ...
    trhf_processed_at = Column(
        Text, nullable=False, comment='Data e hora do processamento do arquivo'
    )
    trhf_prov_id = Column(
        Text, nullable=False, comment='ID do fornecedor que enviou o arquivo'
    )
    ...
    trhf_size_kilobytes = Column(
        Decimal, nullable=False, comment='Tamanho do arquivo em kilobytes'
    )
...
class ProductPrediction(Base):
    ...
    pred_prco_id = Column(Text, nullable=False, comment='ID do produto recomendado')
    pred_clie_id = Column(Text, comment='ID do cliente que comprou o produto')
