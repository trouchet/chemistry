...
def test_get_sets_count_per_items_dict(sample_dataframe):
    sets_count_dict = get_sets_count_per_items_dict(sample_dataframe, 'order_id', 'item_id')
    assert sets_count_dict == {'A': 2, 'B': 2, 'C': 1}

def test_get_items_neighbors_count(sample_dataframe):
    neighbors_count = get_items_neighbors_count(sample_dataframe, 'order_id', 'item_id')
...
def test_get_sets_count_per_items(sample_dataframe):
    counts = get_sets_count_per_items(sample_dataframe, 'order_id', 'item_id')
    df = pd.DataFrame({'item_id': ['A', 'B', 'C'], 'count': [2, 2, 1]})
    
    assert counts.equals(df)

def test_get_sets_to_items_dict(sample_dataframe):
    sets_to_items_dict = get_sets_to_items_dict(sample_dataframe, 'order_id', 'item_id')
